...
5. **승인 후 PR 생성**: 생성된 제목/본문을 사용자에게 보여주고 승인받은 뒤, **현재 브랜치를 head로 main에 머지하는 PR**을 생성한다 (새 브랜치 생성 금지):
   - **승인 요청 방법**: 툴 호출 직전에 출력한 텍스트는 CLI에 표시되지 않으므로, 제목/본문 전체를 **툴 호출 없이 턴의 최종 메시지**로 출력하고 턴을 끝내 사용자 답변("생성"/수정 요청)을 기다린다. AskUserQuestion을 쓰려면 본문을 반드시 옵션의 `preview` 필드에 담는다 — 본문이 안 보이는 채로 승인 질문만 띄우면 안 된다.
